  ...
  if (error && !loading) {
    if (disabledErrorCode !== error.status) {
      switch (error.status) {
        case 400:
          return (
            <IDONATEErrorHandling message="Bad request" code={error.status} />
          );
        case 401:
          return (
            <IDONATEErrorHandling message="Unauthorized" code={error.status} />
          );
        case 404:
          return (
            <IDONATEErrorHandling message="Not Found" code={error.status} />
          );
        case 403:
          return (
            <IDONATEErrorHandling message="Forbidden" code={error.status} />
          );
        case 500:
          return (
            <IDONATEErrorHandling
