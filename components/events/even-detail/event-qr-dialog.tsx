...
export default function EventQrDialog({
  ...
  const { eventName, donor, amount, recipient, acquiringBank, currency, city } =
    paymentData;

  const handleDownload = () => {
    const link = document.createElement("a");
    link.href = qrCodeSrc;
    link.download = `QR_Code_${eventName}.png`;
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose} modal>
      <DialogContent
        className="max-h-[90%] w-full max-w-lg md:min-w-[800px] flex flex-col p-0 overflow-auto scrollbar-hide gap-4 md:gap-6"
      ...
      >
        ...
        <DialogHeader className="sticky top-0 z-50 px-4 md:px-6 py-4 w-full flex flex-row items-center justify-between bg-white shadow-sm">
          <DialogTitle className="p-0 m-0 text-iDonate-navy-primary font-semibold text-lg md:text-2xl">
            Event QR Code
          </DialogTitle>
        ...
        <div className="flex flex-col-reverse md:flex-row items-center justify-center bg-white p-4 md:p-6 rounded-lg shadow-md gap-4 md:gap-6">
          ...
          <div className="flex flex-col gap-3 text-sm text-gray-700 w-full max-w-sm">
            <div className="flex justify-between">
              <span className="font-semibold">Donation Event:</span>
          ...
          {/* QR Code Section */}
          <div className="flex flex-col items-center gap-3 bg-gray-100 p-3 md:p-4 rounded-lg shadow-md">
            <Image
              src={qrCodeSrc}
              width={250}
              height={250}
            ...
            <Button
              ...
              className="mt-2 px-4 py-2 text-white bg-iDonate-navy-secondary  rounded-lg hover:bg-iDonate-navy-primary transition"
            >
              Download QR
            </Button>
          </div>
